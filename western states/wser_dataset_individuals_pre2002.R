...
#### 1980 to 2002
years_pre <- seq(1980,2002, by = 1)
wser_results_pre <- c()
for (k in 1:length(years_pre)) {
  website <- paste0('https://www.wser.org/results/',years_pre[k],'-results/')
  wser_results_pre <- read_html(website) %>% 
    html_nodes('#content') %>%
    map_df(~list(Place = html_nodes(.x,'.column-1') %>%
                   html_text() %>%
                   {if(length(.) == 0){NA} else {.}},
                 Year = rep(years_pre[k],length(html_nodes(.x,'.column-1') %>%
                                              html_text() %>%
                                              {if(length(.) == 0){NA} else {.}})),
                 Time = html_nodes(.x,'.column-2') %>%
                   html_text() %>%
                   {if(length(.) == 0){NA} else {.}},
                 First_Name = html_nodes(.x, '.column-3') %>%
                   html_text() %>%
                   {if(length(.) == 0){NA} else {.}},
                 Last_Name = html_nodes(.x, '.column-4') %>%
                   html_text() %>%
                   {if(length(.) == 0){NA} else {.}},
                 Gender = html_nodes(.x, '.column-5') %>%
                   html_text() %>%
                   {if(length(.) == 0){NA} else {.}},
                 Age = html_nodes(.x, '.column-6') %>%
                   html_text() %>%
                   {if(length(.) == 0){NA} else {.}})) %>%
    rbind(wser_results_pre,.)
}

wser_results_pre$State <- "Unknown"
...
wser_results <- rbind(wser_results_pre, wser_results)
